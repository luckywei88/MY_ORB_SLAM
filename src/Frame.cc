     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    // Scale Level Info
    mnScaleLevels = mpORBextractorLeft->GetLevels();
//    ROS_ERROR("scale leves %d ", mnScaleLevels);
    mfScaleFactor = mpORBextractorLeft->GetScaleFactor();    
//    ROS_ERROR("scale factor %f ", mfScaleFactor);
    mfLogScaleFactor = log(mfScaleFactor);
//    ROS_ERROR("log scale factor %f ", mfLogScaleFactor);
    mvScaleFactors = mpORBextractorLeft->GetScaleFactors();
    mvInvScaleFactors = mpORBextractorLeft->GetInverseScaleFactors();
    mvLevelSigma2 = mpORBextractorLeft->GetScaleSigmaSquares();
    mvInvLevelSigma2 = mpORBextractorLeft->GetInverseScaleSigmaSquares();
...
bool Frame::deletepoints(const cv::Mat& detect)
{
    std::vector<cv::KeyPoint> tmpv;
    cv::Mat tmpDesc; 
    int before=mDescriptors.rows;
	...
	for(i=0;i<2;i++)
	{
	    for(j=0;j<2;j++)
	    {
		int elem=(int)detect.at<unsigned char>(y+i,x+j);
		if(elem!=0)
		    break;
	    }
	    if(j!=2)
	...
	if(i==2&&j==2)
	{
	    cv::Mat tmp=mDescriptors.row(ii);
	    tmpDesc.push_back(tmp);
	    tmpv.push_back(kp);
	}
	ii++;
    }

    int after=tmpDesc.rows;
    mDescriptors=tmpDesc;
    mvKeys=tmpv;
    float diff=(float)after/(float)before;
    cout<<endl<<"before "<<before<<endl;
    cout<<"after "<<after<<endl;
    cout<<"delete "<<diff<<endl;
    return diff<100;
}

void Frame::createXYZRGBPointCloud(const cv::Mat& depth_img,
	const cv::Mat& rgb_img)
{
    cloud=new pointcloud_type();
    cloud->is_dense         = false; //single point of view, 2d rasterized NaN where no depth value was found
    float fxinv,fyinv;
    fxinv=invfx;
    fyinv=invfy;
    int data_skip_step = 2;
